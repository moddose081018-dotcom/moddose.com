...
1. **Payment provider — built, not yet connected.** The Stripe integration lives in
   `worker/` (a Cloudflare Worker) and is wired into the site. It is switched on by setting
   `checkout.endpoint` in `data/site.json` to the deployed Worker URL and rebuilding; until
   then the checkout page says payments are not connected and the button is disabled. See
   [`worker/README.md`](worker/README.md) for deployment, secrets and the pre-launch
   checklist. **It has never been run against Stripe** — do a full test-mode purchase,
   one-time and subscription, before switching to live keys.
...
The site is static; the only server-side component is `worker/`, and it exists so that the
Stripe secret key and the prices live somewhere the browser cannot reach.

The browser posts `{ items: [{slug, qty, plan}], interval }` and nothing else. The Worker
prices the cart from its own generated copy of the catalog, applies the subscription discount
itself, and returns a Stripe Checkout Session URL to redirect to. Card details are entered on
Stripe's page and never touch this site — there is an e2e assertion that the checkout page
contains no card input, in either the connected or unconnected state.

`worker/src/catalog.json` is generated by `npm run build`, so **a price change means
redeploying the Worker as well as the site.** They read the same source; they are deployed
separately.
